...
\Def{}{$h:X\to\mathbb{R},\ X\subset \mathbb{R}^n$ ouvert est $\mathcal{C}^{\infty}$ si $\frac{\partial^ih}{\partial x_1^{i_1}...\partial x_n^{i_n}}$, $i=\sum_{j=1}^n i_j$ existent et sont continues pour tout n-uplet $(i_j)_j$.\\
...
\begin{enumerate}
	...
	\item $\forall T>0; \forall x_0\in\mathbb{R}^n, R_T(x_0)=\mathbb{R}^n$
	\item Rg$(B,...,A^{n-1}B)=n$
\end{enumerate}}
...
où $x(T,u,x_0)$ est la trajectoire de $\dot{x}=F(x,u)$ passant par $x_0$ en $t=0$. 

\Def{}{\ref{Pi} est accessible en $x_0$ si $\widering{R(x_0)}\neq\emptyset$\\
\ref{Pi} est fortement accessible en $x_0$ si $\forall T>0,\ \widering{R_T(x_0)}\neq\emptyset$}
...
\Theo{}{Supposons dim $\mathcal{L}_0(X)=k-1$ \\
