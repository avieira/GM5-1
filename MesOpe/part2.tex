...
\Def{Autoadjoint}{On dit qu'un opérateur $T\in\mathscr{L}(H)$ est autoadjoint si $T^*=T$, ie \[(Tu,v)=(u,Tv)\ \forall u,v\in H\]}
...
\Def{Résolvante et régularisée}{Soit $A$ un opérateur maximal monotone. On pose, pour tout $\lambda>0$, 
...
\Def{}{On définit par récurrence l'espace \[D(A^k)=\{v\in D(A^{k-1});\ Av\in D(A^{k-1})\},\ k \text{ entier }\geq 2\]
